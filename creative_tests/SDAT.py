        ...
        self.language_prompts = {
            "English": 
                (
                    ...
                    f"Make a list of these {str(self.n_words)} words, a single word in each entry of the list. Do not write anything else but the {str(self.n_words)} words." + " "
                ), 
            "Spanish": 
                (
                    ...
                    f"Haz una lista de estas {str(self.n_words)} palabras, una sola palabra en cada entrada de la lista. Solo escribe las {str(self.n_words)} palabras y nada más." + " "
                ), 
            "French": 
                (
                    ...
                    f"Faites une liste de ces {str(self.n_words)} mots, un mot par entrée. N’écrivez rien d’autre que ces {str(self.n_words)} mots." + " "
                ), 
            "German": 
                (
                    ...
                    f"Erstellen Sie eine Liste dieser {str(self.n_words)} Wörter, jeweils ein einzelnes Wort pro Eintrag. Schreiben Sie nichts anderes als die {str(self.n_words)} Wörter." + " "
                ), 
            "Italian": 
                (
                    ...
                    f"Fai un elenco di queste {str(self.n_words)} parole, una sola parola per ogni voce. Non scrivere altro che le {str(self.n_words)} parole." + " "
                ), 
            "Dutch": 
                (
                    ...
                    f"Maak een lijst van deze {str(self.n_words)} woorden, één woord per item. Schrijf niets anders dan de {str(self.n_words)} woorden." + " "
                ), 
            "Portuguese": 
                (
                    ...
                    f"Faça uma lista dessas {str(self.n_words)} palavras, uma palavra por item. Não escreva nada além das {str(self.n_words)} palavras." + " "
                ), 
            "Japanese": 
                (
                    ...
                    f"これらの {str(self.n_words)} 個の単語をリストにし、各項目に1つずつ単語を書いてください。その {str(self.n_words)} 個の単語以外は書かないでください。" + " "
                ), 
            "Arabic":
                (
                    ...
                    f"اكتب قائمة بهذه الكلمات وعددها {str(self.n_words)}، كلمة واحدة في كل مدخل. لا تكتب شيئًا آخر غير هذه {str(self.n_words)} كلمة." + " "
                ),
            "Chinese":
                (
                    ...
                    f"将这 {str(self.n_words)} 个词语列成清单，每一项只写一个词语。除了这 {str(self.n_words)} 个词语之外不要写任何其他内容。" + " "
                ),
            "Korean":
                (
                    ...
                    f"이 {str(self.n_words)}개의 단어를 목록으로 작성하고, 각 항목에는 단어 하나만 적어 주세요. {str(self.n_words)}개의 단어 외에는 아무것도 쓰지 마세요." + " "
                ),
            "Czech":
                (
                    ...
                    f"Vytvořte seznam těchto {str(self.n_words)} slov a do každé položky napište právě jedno slovo. Napište pouze těch {str(self.n_words)} slov a nic dalšího." + " "
                ),
        }
        self.languages = list(self.language_prompts.keys())
    
    def add_addition_specs(self, lang):
        if isinstance(self.addition_specs, dict):
            return self.addition_specs.get(lang, "")
        else:
            return ""
